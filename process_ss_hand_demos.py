...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Extract 3D hand poses from '
                                                 'human hand demos of something-something tasks')
    parser.add_argument('--hand_demos_dir', type=str,
...
def main(args):
    print('Begin processing something-something human hand demos.')

    task_dirs = [osp.join(args.hand_demos_dir, d) for d in os.listdir(args.hand_demos_dir)]
    ...
    for task_dir in tqdm(task_dirs, desc=f'Processing {len(task_dirs)} task directories...'):
        ...
        print(f'Converting frames to bounding boxes.')
        ...
        print(f'Converting bounding boxes to 3D hand poses.')
        ...
        print(f'Filtering data using {args.iou_thresh} IoU threshold.')
        ...
        # save r3m embeddings
        print(f'Saving r3m embeddings from frames.')
