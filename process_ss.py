...
from ss_utils.ss_dataset import (
    generate_vid_list_from_tasks,
...
def parse_args():
    ...
    parser.add_argument('--ss_json_dir', dest='ss_json_dir', type=str, required=True,
                        help='directory to something-something json files',
...
def main(args, task_templates, splits=('train', 'valid')):
    ...
    print(f'The specified tasks are: {task_templates}')
    ...
    for split, vid_dict in dicts.items():
        # convert webm to jpg
        print(f'\nConverting videos to frame images for {split} split.')
        ...
        print(f'\nConverting bounding boxes to 3D hand poses for {split} split.')
        ...
        print(f'\nSaving image and contact info to mocap output pkl files for {split} split.')
        save_info_to_mocap(
            mocap_parent_dir=mocap_output_dir, run_on_cv_server=args.run_on_cv_server,
            save_img_shape=args.save_img_shape, save_contact=args.save_contact
        )
        ...
        if args.save_savgol:
            print(f'\nFiltering contact using Sav-Gol filter and saving results '
                  f'to mocap output pkl files for {split} split.')
            filter_contact_with_savgol(
                mocap_parent_dir=mocap_output_dir,
                savgol_params_path=args.savgol_params_path,
            )

        # filter data by IoU threshold
        print(f'\nFiltering data using {args.iou_thresh} IoU threshold for {split} split.')
        ...
        r3m_command += f"--input_dir={join(args.data_save_dir, args.task_name, split)} "
...
if __name__ == '__main__':
    start = time.time()
    args = parse_args()
    main(args, task_templates=DVD_TASK_TEMPLATES, splits=SPLITS)
    end = time.time()
